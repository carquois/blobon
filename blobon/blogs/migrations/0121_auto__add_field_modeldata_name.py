...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'ModelData.name'
        db.add_column('blogs_modeldata', 'name',
                      self.gf('django.db.models.fields.CharField')(default='Unknown', max_length=140),
                      keep_default=False)

        # Adding M2M table for field relation on 'ModelFieldData'
        db.create_table('blogs_modelfielddata_relation', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('modelfielddata', models.ForeignKey(orm['blogs.modelfielddata'], null=False)),
            ('modeldata', models.ForeignKey(orm['blogs.modeldata'], null=False))
        ))
        db.create_unique('blogs_modelfielddata_relation', ['modelfielddata_id', 'modeldata_id'])


    def backwards(self, orm):
        # Deleting field 'ModelData.name'
        db.delete_column('blogs_modeldata', 'name')

        # Removing M2M table for field relation on 'ModelFieldData'
        db.delete_table('blogs_modelfielddata_relation')
